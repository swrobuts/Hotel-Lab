/**
 * Hotel-Lab · Laufzeit der Lernumgebung
 *
 * Abgeleitet von der WInf-SP-Laufzeit (winf.js). Zustaendig fuer:
 *   - Befehlskarten mit Kopierknopf und Erklaerung der Bestandteile
 *   - eine echte Datenbank im Browser: PostgreSQL (PGlite) mit dem Sternschema
 *     hotel_bi und allen 119.390 Buchungen der Fallstudie
 *   - Uebungsboxen aus data/uebungen/<lab>.json in den Bauformen
 *     quiz, zuordnen, checkliste, sql, json, reihenfolge, regal, deploy
 *   - Regal-Simulator (Power BI) und Deploy-Simulator (Render)
 *   - Fortschrittsanzeige je Lab
 *
 * Nur Deutsch: Die Texte tragen den Schluessel `de`; die Sprachlogik der
 * Vorlage bleibt erhalten, ist aber fest auf Deutsch gestellt.
...
const SPRACHSCHLUESSEL = 'hotel:sprache'
const OSSCHLUESSEL = 'hotel:os'
...
function setzeSprache (lang) {
  ...
  document.dispatchEvent(new CustomEvent('hotel:sprache', { detail: { lang } }))
}
...
function initSprache () {
  ...
  setzeSprache('de')   // Hotel-Lab ist einsprachig
  document.querySelectorAll('[data-lang-btn]').forEach(b => {
...
function setzeOs (os) {
  ...
  document.dispatchEvent(new CustomEvent('hotel:os', { detail: { os } }))
}
...
const T = {
  ...
  dbLaden:      { de: 'Die Datenbank wird gestartet und mit 119.390 Buchungen geladen (einige Sekunden) …', en: 'Starting the database …' },
  dbBereit:     { de: 'PostgreSQL läuft im Browser – Schema hotel_bi mit 119.390 Buchungen', en: 'PostgreSQL is running in your browser' },
  ...
  keineDaten:   { de: 'Noch kein Feld abgelegt.', en: 'Put a field on a shelf.' },
  ...
  typ: {
    ...
    regal:       { de: 'Felder ablegen', en: 'On the shelf' },
...
const DEPLOY_HINWEISE = {
  ...
  dockerfileFehlt:{ de: 'Für die Runtime Docker braucht Render ein Dockerfile im Root Directory.', en: 'For the Docker runtime Render needs a Dockerfile in the root directory.' },
 ...
 */
const LABS = [
  { id: 'lab-00', nr: '00', datei: 'lab-00-fallstudie.html', uebungen: 4,
    titel: { de: 'Fallstudie und Vorgehen' }, voraussetzung: null, zeit: { de: '45 Minuten' },
    ziel: { de: 'Die Fragestellungen der beiden Hotels kennen und den Weg von der Rohdatei bis zum Dashboard in Schritten benennen.' } },
  { id: 'lab-01', nr: '01', datei: 'lab-01-datenquelle.html', uebungen: 5,
    titel: { de: 'Die Datenquelle' }, voraussetzung: { de: 'Lab 00' }, zeit: { de: '60 Minuten' },
    ziel: { de: 'Die 32 Spalten des Datensatzes lesen, ihre Typen und Eigenheiten kennen und wissen, woher die Daten stammen.' } },
  { id: 'lab-02', nr: '02', datei: 'lab-02-colab.html', uebungen: 5,
    titel: { de: 'Google Colab' }, voraussetzung: null, zeit: { de: '75 Minuten' },
    ziel: { de: 'Ein Notebook so führen, dass es von oben nach unten reproduzierbar durchläuft.' } },
  { id: 'lab-03', nr: '03', datei: 'lab-03-notebook.html', uebungen: 5,
    titel: { de: 'Das Notebook Schritt für Schritt' }, voraussetzung: { de: 'Lab 01, Lab 02' }, zeit: { de: '90 Minuten' },
    ziel: { de: 'Laden, Sichten, Bereinigen, Kennzahlen und Diagramme des Notebooks nachvollziehen und jede Zelle erklären können.' } },
  { id: 'lab-04', nr: '04', datei: 'lab-04-sternschema.html', uebungen: 5,
    titel: { de: 'Das Sternschema' }, voraussetzung: { de: 'Lab 01' }, zeit: { de: '75 Minuten' },
    ziel: { de: 'Fakten von Dimensionen trennen, Schlüssel erkennen und eine Kennzahl über einen Join aus dem Stern holen.' } },
  { id: 'lab-05', nr: '05', datei: 'lab-05-datenbank.html', uebungen: 5,
    titel: { de: 'Datenbank und Zugang' }, voraussetzung: { de: 'Lab 04' }, zeit: { de: '60 Minuten' },
    ziel: { de: 'Sich mit der Datenbank hotel verbinden, die Rechte der Rolle studi_hotel kennen und Abfragen sicher absetzen.' } },
  { id: 'lab-06', nr: '06', datei: 'lab-06-kennzahlen.html', uebungen: 5,
    titel: { de: 'Kennzahlen' }, voraussetzung: { de: 'Lab 04, Lab 05' }, zeit: { de: '75 Minuten' },
    ziel: { de: 'Die zehn Kennzahlen des Katalogs definieren, in SQL nachrechnen und nach ihrer Wirkung auf das Ergebnis färben.' } },
  { id: 'lab-07', nr: '07', datei: 'lab-07-powerbi.html', uebungen: 6,
    titel: { de: 'Dashboard in Power BI und Tableau' }, voraussetzung: { de: 'Lab 04, Lab 06' }, zeit: { de: '120 Minuten' },
    ziel: { de: 'Ein Management-Dashboard vom Konzeptblatt bis zur Veröffentlichung bauen – in Power BI mit Measures und Slicern, in Tableau mit berechneten Feldern und Parametern.' } },
  { id: 'lab-08', nr: '08', datei: 'lab-08-dashboard.html', uebungen: 5,
    titel: { de: 'Das Dashboard' }, voraussetzung: { de: 'Lab 06' }, zeit: { de: '75 Minuten' },
    ziel: { de: 'Den Aufbau des Dashboards aus Backend und Frontend erklären, die Gestaltungsregeln anwenden und den Dienst auf Render bringen.' } },
  { id: 'lab-09', nr: '09', datei: 'lab-09-befunde.html', uebungen: 4,
    titel: { de: 'Vom Befund zur Handlungsempfehlung' }, voraussetzung: { de: 'Lab 06, Lab 08' }, zeit: { de: '60 Minuten' },
    ziel: { de: 'Aussagen und Abweichungen des Dashboards lesen, deuten und in begründete Empfehlungen übersetzen.' } }
]
...
function zwei (knoten, wert, eigenschaft = 'textContent') {
  ...
  setze()
  document.addEventListener('hotel:sprache', setze)
  return knoten
}
...
function baueBefehl (ziel, def) {
  ...
  zeichne()
  document.addEventListener('hotel:os', zeichne)
  document.addEventListener('hotel:sprache', zeichne)
...
function baueTerminal (ziel, opt = {}) {
  ...
  if (!fest) {
    document.addEventListener('hotel:os', () => {
      welt.os = aktuellesOs()
      zeichneKopf()
  ...
  document.addEventListener('hotel:sprache', zeichneAuftrag)
...
/* ================================================================ Datenbank
   ...
   PostgreSQL kommt als PGlite (WebAssembly) und traegt das Sternschema hotel_bi
   der Fallstudie. Eine Seite startet die Datenbank ueber das Attribut
   data-datenbank="postgres". Ergebnisse haben immer die Form { fields, rows }
   mit Zeilen als Felder - damit gleichnamige Spalten nicht zusammenfallen.
   =========================================================================== */

const MASCHINEN = {
  postgres: { bereit: 'dbBereit', fehler: 'fehlerSql' }
}
let seitenMaschine = 'postgres'
const dbVersprechen = {}
const dateien = {}
// Reihenfolge wie beim Laden auf dem Server: erst die Dimensionen, dann die Fakten.
const TABELLEN = ['dim_hotel', 'dim_date', 'dim_market_segment', 'dim_distribution_channel',
...
async function holeDatei (pfad) {
  if (dateien[pfad] == null) {
    const r = await fetch(url(pfad))
    if (!r.ok) throw new Error(pfad + ': ' + r.status)
    dateien[pfad] = await r.text()
  }
  return dateien[pfad]
}

/**
 * Legt das Sternschema hotel_bi an und laedt die neun CSV-Dateien per COPY -
 * derselbe Bestand wie in der Datenbank hotel auf dem Server (119.390 Buchungen).
 * Alle selbst angelegten Schemata fallen vorher, damit eine Pruefung immer auf
 * demselben Ausgangsbestand rechnet. Der Suchpfad steht danach wie fuer die
 * Rolle studi_hotel auf hotel_bi.
 */
async function saeen (h) {
  ...
  for (const tabelle of TABELLEN) {
    const csv = await holeDatei(`data/${tabelle}.csv`)
    const kopf = csv.slice(0, csv.indexOf('\n')).trim()
    await h.db.query(`COPY hotel_bi.${tabelle} (${kopf}) FROM '/dev/blob' WITH (FORMAT csv, HEADER true)`, [], { blob: new Blob([csv]) })
  }
...
function baueDbBand (ziel, maschine) {
  ...
  const setzen = async () => {
    ...
    try {
      ...
      btn.disabled = false
      document.dispatchEvent(new CustomEvent('hotel:datenbank'))
    } catch (e) {
...
function hinweisUndLoesung (koerper, uebung, eingabe) {
  if (uebung.hinweis) {
    ...
    zeichne(); document.addEventListener('hotel:sprache', zeichne)
    d.append(inh)
    koerper.append(d)
  }
  if (uebung.loesung) {
...
const REGAL_TEXTE = {
  ...
  powerbi: {
    ...
    felder: 'Data', dimension: { de: 'Spalte (Text/Datum)', en: 'column (text/date)' }, kennzahl: { de: 'Σ Zahlenfeld', en: 'Σ measure' }
  }
}
...
const FARBEN = ['#2E3238', '#C0662B', '#5F7F95', '#8C4A1F', '#8C9A5B', '#A7A9AD', '#D9A06B']
...
function baueRegal (ziel, opt) {
  ...
  document.addEventListener('hotel:sprache', () => { zeichneFelder(); zeichneAlles() })
  return { belegung }
}
...
function baueBox (uebung, ctx) {
  ...
  zeichneAufgabe()
  document.addEventListener('hotel:sprache', zeichneAufgabe)
...
function initTitel () {
  const en = document.querySelector('meta[name="hotel:titel-en"]')?.content
...
function karteFortschritt (nurLab = null) {
  ...
  zeichne()
  document.addEventListener('hotel:fortschritt', zeichne)
  document.addEventListener('hotel:sprache', zeichne)
}
...
async function starteLab (labId) {
  ...
  // Datenbankband, falls die Seite SQL enthaelt (genau einmal je Seite).
  const dbHalter = document.querySelector('[data-datenbank]')
  if (dbHalter) baueDbBand(dbHalter, seitenMaschine)
  ...
  for (const halter of document.querySelectorAll('[data-sql-konsole]')) {
    const box = baueBox({
      ...
      start: halter.dataset.start || 'SELECT * FROM dim_hotel;',
  ...
  for (const halter of document.querySelectorAll('[data-json-konsole]')) {
    const box = baueBox({
      ...
      start: halter.dataset.start || '{ "hotel": "Resort Hotel", "lead_time": 342, "is_canceled": false }'
    }, { lab: labId, fortschritt: {} })
    box.querySelector('.uebung-kopf').remove()
    box.className = 'sql-konsole'
    halter.replaceChildren(box)
